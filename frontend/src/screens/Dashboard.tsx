  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-6">
        ...
        <div
          className={`px-3 py-1.5 rounded-xl text-sm font-black ${statusColor}`}
        >
      ...
      <button
        onClick={onView}
        className="w-full mt-4 py-2.5 rounded-xl bg-surface-900 text-white text-sm font-bold hover:bg-black transition-all"
      >
        View Attendance
      </button>
    </div>
...
const Dashboard = () => {
  ...
  const totalAttended = subjects.reduce((sum, s) => sum + s.presentCount, 0);
  const totalClasses = subjects.reduce((sum, s) => sum + s.classesTillNow, 0);
    ...
    <div className="space-y-8 animate-fade-in pb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 relative overflow-hidden bg-surface-900 rounded-[2rem] p-8 text-white shadow-bento group">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-xs font-bold text-white/90 mb-6 border border-white/10 uppercase tracking-wider">
              <Sparkles className="w-3.5 h-3.5 text-primary-300" /> Current
              Standing
            </div>
        ...
        <div className="md:col-span-1 bg-gradient-to-br from-primary-500 to-primary-700 rounded-[2rem] p-8 text-white shadow-bento flex flex-col justify-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="text-primary-100 font-bold uppercase tracking-wider text-sm mb-2">
              Target
            </div>
            <div className="text-5xl font-black mb-4">
              {requiredPercentage}%
            </div>
            <p className="text-primary-100/80 text-sm font-medium leading-relaxed">
              Keep your overall attendance above this line to stay safe.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {subjects.map((subject) => (
            <DashboardCard
              ...
              onView={() => navigate(`/app/attendance/subject/${subject.id}`)}
            />
          ))}
        </div>
      </div>
