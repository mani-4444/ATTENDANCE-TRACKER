...
const PredictorCard = ({ title, value, colorClass }: { title: string; value: number | string; colorClass: string }) => (
  <div className={`p-6 rounded-3xl flex flex-col justify-center items-center shadow-bento ${colorClass}`}>
    <div className="text-xs font-bold opacity-80 mb-2 leading-tight text-center uppercase tracking-wider">{title}</div>
    <div className="text-4xl sm:text-5xl font-black tracking-tight">{value}</div>
  </div>
);
...
const Predictor = () => {
  ...
  const [targetPercent, setTargetPercent] = useState<number>(requiredPercentage);
  
  const isOverall = selectedSubjectId === 'overall';
  const subject = isOverall ? null : subjects.find(s => s.id === selectedSubjectId);

  if (!isOverall && !subject) return <div className="p-4 text-center font-medium text-gray-500">No subjects available</div>;

  const reqDecimal = targetPercent / 100;
  
  const hypotheticalTotal = isOverall 
    ? subjects.reduce((sum, s) => sum + s.total, 0) || 1
    : subject!.total || 1;
  ...
  const classesNeededToAttend = Math.max(0, targetRequiredCount - currentAttended);
  ...
  return (
    <div className="animate-fade-in pb-10 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
          ...
          <div className="relative group">
            ...
            >
              <option value="overall" className="font-sans font-medium">Overall</option>
              {subjects.map(s => (
                <option key={s.id} value={s.id} className="font-sans font-medium">{s.name}</option>
              ))}
            </select>
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
          ...
          <div className="relative group">
            ...
            >
              <option value={75} className="font-sans font-medium">75%</option>
              <option value={80} className="font-sans font-medium">80%</option>
              <option value={85} className="font-sans font-medium">85%</option>
              <option value={90} className="font-sans font-medium">90%</option>
            </select>
            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-6 text-gray-400 group-hover:text-primary-500 transition-colors">
              <ChevronDown className="w-6 h-6" strokeWidth={2.5} />
            </div>
      ...
      {/* Main Status */}
      <div className={`p-8 md:p-12 rounded-[2rem] border overflow-hidden relative text-center ${statusColor}`}>
        <div className={`absolute top-0 right-0 w-64 h-64 bg-gradient-to-br ${statusGradient} opacity-5 blur-3xl rounded-full translate-x-1/2 -translate-y-1/2`}></div>
        <div className={`absolute bottom-0 left-0 w-64 h-64 bg-gradient-to-tr ${statusGradient} opacity-5 blur-3xl rounded-full -translate-x-1/2 translate-y-1/2`}></div>
        
        <div className="relative z-10 flex flex-col items-center justify-center">
           <div className="mb-6 bg-white p-4 rounded-3xl shadow-sm border border-white/50 inline-block">
             {statusIcon}
           </div>
           <h3 className="text-4xl md:text-5xl font-black tracking-tight mb-4">{statusText}</h3>
